"""
violence_tracker.py
This file is responsible for detecting violence against women and lone women presence using YOLO for object tracking and CLIP for action/gender classification. It also sends WhatsApp alerts and logs incidents.

🔹 Key Functionalities
Detects people in video frames using YOLOv8.
Identifies gender and actions using CLIP.
Detects violence against women by analyzing actions & interactions.
Detects if a woman is alone in the frame.
Logs incidents to CSV & saves screenshots for records.
Sends WhatsApp alerts with Cloudinary-uploaded images.
...
class ViolenceTracker:
    ...
    def send_whatsapp_alert(self, violence_against_women, female_count, image_url):
        ...
        # Skip sending alert if violence against women is detected
        if violence_against_women:
            print("Violence against women detected. No alert sent.")
            return
